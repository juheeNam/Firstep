...
- default export 대신 named export 사용 (단, Next.js 규약상 `page.tsx` / `layout.tsx` / `error.tsx` / `loading.tsx` / `not-found.tsx` 는 default export 필수)
- 컴포넌트는 함수형 + hooks
- API 응답은 항상 타입 정의
...
- 커밋 메시지·PR 제목·PR 본문: **한국어**로 작성, 커밋 접두사 사용 (feat:, fix:, docs:, refactor:, chore:)
...
- **PR 제목과 본문은 반드시 한국어로 작성한다** — `## Summary`, `## Test plan` 등 섹션 헤더는 그대로 두되, 그 안의 모든 불릿/문장은 한국어. 변경 이유, 변경 내용, 테스트 방법을 포함한다.
- 승인 없이 merge 하지 않는다

### PR 자동 생성 규칙

아래 조건이 모두 충족되면 **유저 확인 없이 PR 을 즉시 생성**한다. 유저에게는 merge 요청만 한다.

1. `npm run lint` / `npx tsc --noEmit` / `npm run build` 모두 통과
2. dev 스모크 테스트 통과 (라우트·플로우 변경 시)
3. `qa-tester` 서브에이전트 리뷰 결과 🔴 치명 이슈 0개
4. 작업 브랜치가 원격에 푸시되어 있음

충족되지 않으면 먼저 수정/검증부터 수행하고, 실패가 반복되면 유저에게 보고한다.
